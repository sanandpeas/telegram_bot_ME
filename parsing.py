    ...
    while True:
        ...
        for adr in range(len(mint_addresses)):
            ...
            response1 = requests.get(
                url='https://api-mainnet.magiceden.dev/rpc/getNFTByMintAddress/' + mint_addresses[adr],
                headers={
                    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) '
                                  'Chrome/109.0.0.0 Safari/537.36',
                    "Accept": "application/json, text/plain, */*",
            ...
            if float(result['results']['price']) < float(f'{price}') and trait_.get(main_trait) == type_trait:
                results.append({'link on site': 'https://magiceden.io/item-details/' + result['results'][
                    'mintAddress'] + '?name=' + result['results']['attributes'][0]['value'],
                ...
                print(trait_)
                print('https://magiceden.io/item-details/' + result['results']['mintAddress'] + '?name=' +
                      result['results']['attributes'][0]['value'])
